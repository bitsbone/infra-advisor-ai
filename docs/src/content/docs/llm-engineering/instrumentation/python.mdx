...
This content-rich mode is limited to services whose data policy explicitly permits it. The MCP provider-adapter deployment forces `DD_LLMOBS_ENABLED=false`; it retains ordinary APM/HTTP timing and explicit bounded metrics but prevents automatic MCP, LangChain, OpenAI, and embedding payload extraction. Do not enable automatic LLMObs on that service unless its telemetry policy and tests are intentionally changed first.
