...
MAUI Release builds retain portable PDBs, generate Android R8 mappings, and generate iOS dSYMs. Symbol upload is opt-in through `DatadogUploadSymbols=true` and requires `DATADOG_API_KEY` only in the release environment. A separate signed APK or development/ad-hoc IPA can be synchronized to a manually created Mobile App Testing application with `datadog-ci`; the Mobile Application ID is distinct from the public RUM application ID.
