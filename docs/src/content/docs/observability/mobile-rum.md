---
title: Mobile RUM
description: Native and .NET MAUI RUM, Session Replay, logs, error tracking, resource monitoring, multimodal AI, and distributed tracing into the InfraAdvisor AI backend
---

InfraAdvisor AI includes native iOS/Android applications and a shared .NET MAUI application that demonstrate how a client interaction becomes a correlated RUM action, network resource, mobile span, backend APM trace, and AI response. They also demonstrate Session Replay privacy, safe logs/errors, multimodal uploads, crash capture, and release symbol upload so Error Tracking shows actionable mobile stack frames. All clients use the deployed backend contracts without changing the server for mobile-specific behavior.

The source lives under `mobile/native/ios`, `mobile/native/android`, and `mobile/cross-platform/maui`. The reserved `mobile/cross-platform/react-native` directory keeps the platform boundary explicit for a future example.
...
## .NET MAUI automatic instrumentation and multimodal AI

The .NET 10 MAUI project targets Android API 23+ and iOS 15+ from one XAML/MVVM application. It provides Login, Chat, Errors, and Info destinations; a phone history bottom sheet and persistent wide-screen history rail; a Syncfusion segmented Python/.NET backend selector and model picker; fragmented SSE streaming; tool/pipeline progress; prompt suggestions; citations and trace metadata; response feedback; and one image plus one audio attachment per turn. The federal procurement suggestion is designed to exercise an MCP call, while image and recorded WAV inputs make it possible to follow vision or transcription work downstream from the mobile resource.

`MauiProgram.cs` enables `Datadog.Maui` Core, Logs, Trace, RUM, and Session Replay. It configures only `infra-advisor-ai.kyletaylor.dev` as first party, propagates Datadog and W3C headers, samples demo RUM sessions/resources/replays at 100%, collects native crashes, and selects `TextAndInputPrivacy.MaskSensitiveInputs`. Named `[DdView]` pages and stable automation IDs keep RUM and UI-test names understandable even if view-model types change.

The application uses one typed, DI-managed `HttpClient` rather than wrapping each operation with custom resources. Datadog automatically creates the resource and client span around login, models, suggestions, conversations, media uploads, SSE queries, feedback, and Error Lab failures. Application headers carry the stable AI session, RUM session, user ID, and conversation ID needed for memory, AI observability correlation, and persistence. Custom telemetry stays at the higher-level workflow boundary and records only controlled operation names, backend, modality, byte size, status, and error type. Event mappers remove URL queries/fragments, redact prompt-like automatic action names, sanitize diagnostic URLs, and reject sensitive custom attribute keys before resources, actions, errors, or logs are submitted.

File and microphone access is user initiated. The picker restricts supported image/audio types, validation rejects unsupported MIME values and files over 10 MB before upload, microphone permission is requested only when Record is tapped, and recordings use WAV in the OS cache. Neither local filenames, paths, raw media, signed upload URLs, prompts, answers, nor response bodies are added to logs, RUM attributes, or spans.

The MAUI Info view explains the active public configuration without showing the client token or JWT. Logout drops the in-memory token, clears the Datadog user, stops the current RUM session, and returns to Login. Error Lab emits fixed safe logs, reports a handled C# exception, performs an intentionally missing API request through the normal instrumented client, and uses a genuine process termination for crash testing. Relaunch the debugger-free application after a crash so the SDK can deliver the stored report.

MAUI Release builds retain portable PDBs, generate Android R8 mappings, and generate iOS dSYMs. The pinned community package `kyletaylored.Datadog.MAUI.Symbols` uploads matching native symbol artifacts after `Publish`; upload is opt-in through `DatadogSymbolUploadEnabled=true` and requires `DATADOG_API_KEY` only in the release environment. A separate signed APK or development/ad-hoc IPA can be synchronized to a manually created Mobile App Testing application with `datadog-ci`; the Mobile Application ID is distinct from the public RUM application ID.

See the [MAUI source guide](https://github.com/kyletaylored/infra-advisor-ai/tree/main/mobile/cross-platform/maui) for file-by-file integration links and Android/iOS run commands.
