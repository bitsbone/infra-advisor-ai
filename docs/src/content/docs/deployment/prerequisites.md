...
`make check-env` is authoritative for the baseline cluster deployment. It currently validates Azure OpenAI/Search/Storage, provider keys, Datadog, registry identity, PostgreSQL, authentication, and Mailpit credentials.
...
- Kubernetes secrets (including `ghcr-pull-secret`) are namespace-scoped; create them per namespace.
