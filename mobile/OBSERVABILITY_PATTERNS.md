...
The MAUI observability facade deliberately exposes only controlled logs/errors, user association, and session lifecycle. `TelemetrySanitizer` filters log and RUM attributes, removes query strings/fragments from resource and diagnostic URLs, and replaces prompt-like automatic action names before data leaves the process. Login calls `DdSdk.SetUserInfo`; logout cancels active chat/media work, clears the in-memory JWT, calls `DdSdk.ClearUserInfo`, and stops the current RUM session. Attachment events record modality, byte size, status, and duration only. The Error Lab uses a handled exception, a missing authenticated API route, fixed logs, and `Environment.FailFast` for a genuine process crash; relaunch is required for stored crash delivery.

Release builds preserve portable PDB information, enable Android R8 mapping output, and generate iOS dSYMs. The pinned `kyletaylored.Datadog.MAUI.Symbols` package discovers and uploads those artifacts after `Publish`; its `DatadogSymbolUploadEnabled` MSBuild property remains off by default and is enabled only in an authorized release job where `DATADOG_API_KEY` is supplied from a secret store. Mobile App Testing upload remains a separate `datadog-ci synthetics upload-application` step using a manually created platform application ID and a signed APK or development/ad-hoc IPA.
