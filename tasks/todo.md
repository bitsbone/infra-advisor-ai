...
- [ ] Run Azure deployment validation/what-if and compliance queries when credentials and permissions are available; do not deploy, assign policy, create RBAC grants, remediate, or mutate the shared subscription without explicit authorization. **Not done** — no Azure credentials available in this session/environment. Run `az deployment sub what-if` against both templates before any real deployment.
...
- Rollout state: **not deployed**. `requiredTagsEffect` defaults to `Audit`; switching to `Deny` is an explicit, separate future redeploy per `tag-governance.md`'s rollout order.
- Remaining Azure permission requirements: subscription `Owner` or (`Resource Policy Contributor` + `User Access Administrator`) to deploy the governance template; the remediation identity itself only gets Tag Contributor.
- Exclusions/exemptions: `rg-tola-infra-advisor-ai-nodes` (this project's AKS node RG) via `exemptResourceGroupNames`.
- Resources confirmed non-taggable by design (not by an Azure API response, since nothing has been deployed live yet): AKS's node resource group, its VMSS, and its NICs — all owned by the AKS resource provider's own control loop.
- Two syntax choices flagged as unverified against a live subscription and worth confirming before enabling `Deny` or running real remediation: the `tags[parameters('tagName')]` field-expression syntax in both custom policy definitions, and the Tag Contributor role GUID (`4a9ae827-6dc8-4573-8ac7-8239d42aa03f`) in `inherit-tags-modify.bicep`.
